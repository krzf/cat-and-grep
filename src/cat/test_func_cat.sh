...
testing()
{
    ...
    ./Mycat $t > test_Mycat.log
    cat $t > test_sys_cat.log
    DIFF_RES="$(diff -s test_Mycat.log test_sys_cat.log)"
    (( COUNTER++ ))
    if [ "$DIFF_RES" == "Files test_Mycat.log and test_sys_cat.log are identical" ]
    then
      (( SUCCESS++ ))
    ...
    rm test_Mycat.log test_sys_cat.log
}
